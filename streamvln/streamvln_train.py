...
def train_with_continual_learning_5d(trainer, training_args, data_module):
    ...
    # Trainable-only weight decay (safe wrt Theorem 1; see decay_trainable).
    # Pulls back cold-start adapters that diverge to large-magnitude mode
    # collapse without shrinking the already-tiny working adapters. 0 = off.
    tucker_trainable_wd = float(getattr(training_args, "tucker_trainable_wd", 0.0) or 0.0)
    if training_args.local_rank in (-1, 0):
        rank0_print(f"[TuKA-5D] trainable-only weight decay = {tucker_trainable_wd}")

    # Persistent state file so successive torchrun invocations share growth state
    state_path = training_args.expansion_state_path
    if state_path is None:
        state_path = os.path.join(training_args.output_dir, "tucker5d_state.json")

    mgr = TaskExpansionManager(
        model=trainer.model,
        state_path=state_path,
        delta_r1=training_args.delta_r1,
        delta_r2=training_args.delta_r2,
        delta_r3=training_args.delta_r3,
        delta_r4=training_args.delta_r4,
        delta_r5=training_args.delta_r5,
        expand_on_any_new=True,
        verbose=(training_args.local_rank in (-1, 0)),
    )

    s_row, e_row, p_row = mgr.begin_task(
        scene_idx=training_args.current_scene_idx,
        env_idx=training_args.current_env_idx,
        instr_idx=training_args.current_instr_idx,
    )
    ...
    # BEFORE trainer.train(), so the optimizer hasn't been created yet --
    # LLaVATrainer builds it on first .train() call. OK.

    # Trainer callback: mask gradients + re-zero pad after each step
    class Expansion5DCallback(transformers.TrainerCallback):
        ...
        def on_step_end(self, args, state, control, **kwargs):
            # Trainable-only weight decay BEFORE re-zeroing pads: shrinks the
            # newly-trained U1/U2 columns + non-frozen-corner G to keep the
            # adapter a small residual (prevents cold-start divergence). Frozen
            # factors are untouched, so Theorem 1 stays exact.
            decay_trainable_all(self.model, tucker_trainable_wd)
            # Re-zero zero-padded positions after optimizer step (guards
            # against weight-decay / momentum drift).
            enforce_zero_pad_all(self.model)
            # Keep routing in place (some trainers reset state between steps)
            self.model.set_tucker5d_route(s_row, e_row, p_row)
            if state.global_step % max(args.logging_steps, 1) == 0:
                ...
                for log_entry in reversed(state.log_history):
                    if isinstance(log_entry, dict) and 'loss' in log_entry:
                        ...
                        break
                rank0_print(f"[TuKA-5D] Task scene={training_args.current_scene_idx} "
                            f"env={training_args.current_env_idx} "
                            f"instr={training_args.current_instr_idx} "
                            f"step={state.global_step} loss={loss_value}")
            return control

    trainer.add_callback(Expansion5DCallback(trainer.model))

    rank0_print("=" * 60)
    rank0_print(f"[TuKA-5D] Starting task  scene={training_args.current_scene_idx}  "
                f"env={training_args.current_env_idx}  instr={training_args.current_instr_idx}")
    rank0_print(f"[TuKA-5D] Route rows: s={s_row}, e={e_row}, p={p_row}")
    rank0_print(f"[TuKA-5D] Current ranks: r1={mgr.state.r1}, r2={mgr.state.r2}, "
                f"r3={mgr.state.r3}, r4={mgr.state.r4}, r5={mgr.state.r5}")
    rank0_print(f"[TuKA-5D] Trainable params: {mgr.num_trainable_params():,}")
    rank0_print("=" * 60)

    try:
        trainer.train()
    except Exception as e:
        rank0_print(f"[TuKA-5D] Training error: {e}")
        import traceback
        traceback.print_exc()
        raise

    # Commit the current shape as frozen so the next task preserves these deltas
    mgr.end_task()

    # Save per-task 5D weights so they can be reloaded for inference
    tucker_5d_dir = training_args.tucker_5d_dir or os.path.join(training_args.output_dir, "tucker_5d")
    if training_args.local_rank in (-1, 0):
        os.makedirs(tucker_5d_dir, exist_ok=True)
        tucker_state = {}
        for name, layer in trainer.model.tucker_5d_layers.items() if hasattr(trainer.model, "tucker_5d_layers") else []:
            tucker_state[name] = {
                "U1": layer.U1.detach().cpu(),
                "U2": layer.U2.detach().cpu(),
                "U3": layer.U3.detach().cpu(),
                "U4": layer.U4.detach().cpu(),
                "U5": layer.U5.detach().cpu(),
                "G":  layer.G.detach().cpu(),
                # CRITICAL: persist r1_initial. scaling = lora_alpha / r1_initial
                # is used both during training and at eval. If we don't save it,
                # eval reconstruction infers r1_initial from CURRENT r1 (~52)
                # instead of the training-time initial r1 (16) and scaling
                # comes out 3x too small, garbling the LM output.
                "r1_initial": layer.r1_initial,
                "r1": layer.r1, "r2": layer.r2, "r3": layer.r3,
                "r4": layer.r4, "r5": layer.r5,
                "frozen_r1": int(layer.frozen_r1.item()),
                "frozen_r2": int(layer.frozen_r2.item()),
                "frozen_r3": int(layer.frozen_r3.item()),
                "frozen_r4": int(layer.frozen_r4.item()),
                "frozen_r5": int(layer.frozen_r5.item()),
                "frozen_rows_U3": int(layer.frozen_rows_U3.item()),
                "frozen_rows_U4": int(layer.frozen_rows_U4.item()),
                "frozen_rows_U5": int(layer.frozen_rows_U5.item()),
            }
        snapshot_path = os.path.join(
            tucker_5d_dir,
            f"tucker5d_task{training_args.current_task_id}_s{training_args.current_scene_idx}"
            f"_e{training_args.current_env_idx}_p{training_args.current_instr_idx}.pt",
        )
        torch.save({"layers": tucker_state, "manager_state": mgr.state.as_dict()}, snapshot_path)
        rank0_print(f"[TuKA-5D] Saved task snapshot -> {snapshot_path}")
        # Also save 'latest' for easy resumption
        latest_path = os.path.join(tucker_5d_dir, "tucker5d_latest.pt")
        torch.save({"layers": tucker_state, "manager_state": mgr.state.as_dict()}, latest_path)
        rank0_print(f"[TuKA-5D] Saved latest -> {latest_path}")

    rank0_print(f"[TuKA-5D] Task {training_args.current_task_id} training completed")
    return trainer


def load_tucker5d_snapshot(model, snapshot_path: str, strict_shape: bool = False):
    """
    Load a 5D Tucker-LoRA snapshot (saved by train_with_continual_learning_5d)
    into the current `model`. If the stored ranks are larger than what the
    model currently has, the model's factors are grown in-place to match
    before loading.
    """
    from streamvln.model.tucker_5d_lora_layers import Tucker5DLoRALinear
    ...
    loaded = 0
    for name, module in model.named_modules():
        if not isinstance(module, Tucker5DLoRALinear):
            continue
        ...
        # Grow model factors to match snapshot ranks
        dr1 = s["r1"] - layer.r1
        dr2 = s["r2"] - layer.r2
        dr3 = s["r3"] - layer.r3
        dr4 = s["r4"] - layer.r4
        dr5 = s["r5"] - layer.r5
        if dr1 > 0 or dr2 > 0 or dr3 > 0 or dr4 > 0 or dr5 > 0:
            layer.expand(max(dr1, 0), max(dr2, 0), max(dr3, 0), max(dr4, 0), max(dr5, 0))

        # Append missing U3/U4/U5 rows
        for axis, U_saved in (("U3", s["U3"]), ("U4", s["U4"]), ("U5", s["U5"])):
            cur_rows = getattr(layer, axis).shape[0]
            want_rows = U_saved.shape[0]
            for _ in range(want_rows - cur_rows):
                layer.append_category_row(axis, init="zeros")

        # Now shapes match -> copy
        with torch.no_grad():
            layer.U1.data.copy_(s["U1"].to(layer.U1.device, dtype=layer.U1.dtype))
            layer.U2.data.copy_(s["U2"].to(layer.U2.device, dtype=layer.U2.dtype))
            layer.U3.data.copy_(s["U3"].to(layer.U3.device, dtype=layer.U3.dtype))
            layer.U4.data.copy_(s["U4"].to(layer.U4.device, dtype=layer.U4.dtype))
            layer.U5.data.copy_(s["U5"].to(layer.U5.device, dtype=layer.U5.dtype))
            layer.G.data.copy_(s["G"].to(layer.G.device, dtype=layer.G.dtype))
            layer.frozen_r1.fill_(s["frozen_r1"])
            layer.frozen_r2.fill_(s["frozen_r2"])
            layer.frozen_r3.fill_(s["frozen_r3"])
            layer.frozen_r4.fill_(s["frozen_r4"])
            layer.frozen_r5.fill_(s["frozen_r5"])
            layer.frozen_rows_U3.fill_(s["frozen_rows_U3"])
            layer.frozen_rows_U4.fill_(s["frozen_rows_U4"])
            layer.frozen_rows_U5.fill_(s["frozen_rows_U5"])
        loaded += 1
    return loaded, ckpt.get("manager_state", {})

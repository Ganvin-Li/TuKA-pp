...
class StreamVLNForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
            ...
            for i in range(num_specials + 1):  # num_images = 1? [0, 1]
                ...
                if i < num_specials:
                    ...
                    if special_token == IMAGE_TOKEN_INDEX:
                        ...
                        # Defensive guard: a <memory> token can be present in the
                        # input while no memory feature is available for this
                        # batch entry (memory_features[batch_idx] is None when the
                        # generation's image batch had num_view==1, e.g. on
                        # pathological long/runaway episodes where the history /
                        # KV-cache bookkeeping desyncs). Without this guard the
                        # next line raises `'NoneType' object is not subscriptable`
                        # and the whole eval rank dies (which then NCCL-times-out
                        # its siblings, losing the entire task's remaining eps).
                        # Skip the memory token instead -- the surrounding text is
                        # already preserved; only the (unavailable) memory embed
                        # is dropped.
                        mem_b = memory_features[batch_idx] if memory_features is not None else None
                        if mem_b is None or cur_mem_id >= len(mem_b):
                            cur_mem_id += 1
                            continue
                        cur_memory_feature = mem_b[cur_mem_id]
                        cur_mem_id += 1
    ...
    # ------------------------------------------------------------------
    # 5D Tucker-LoRA routing helpers
    # ------------------------------------------------------------------
    def set_tucker5d_route(self, scene_row: int, env_row: int, instr_row: int):
        """
        Broadcast the hard (scene_row, env_row, instr_row) triple to every
        Tucker5DLoRALinear adapter in the model. Must be called BEFORE any
        forward / generate that should use the corresponding task's delta.
        """
        try:
            from streamvln.model.tucker_5d_lora_layers import set_active_route_all
        except ImportError:
            from .tucker_5d_lora_layers import set_active_route_all
        set_active_route_all(self, scene_row, env_row, instr_row)
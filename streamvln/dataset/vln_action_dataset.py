...
class VLNActionDataset(Dataset):
    def __init__(
        ...
        # --- robustness options: filter episodes whose rgb folder is missing / too few frames ---
        # Set to False if you fully trust the annotation file, or to speed up dataset init.
        self.check_video_dir = getattr(data_args, 'check_video_dir', True)
        min_frames = getattr(data_args, 'min_frames', 4)
        ...
        for ep_id, item in enumerate(self.nav_data):
            ...
            # Optional: verify the rgb directory actually exists and has enough frames on disk.
            # This prevents IndexError later when video_frames is shorter than actions_len.
            if self.check_video_dir:
                rgb_dir = os.path.join(item['video'], 'rgb')
                if not os.path.isdir(rgb_dir):
                    skipped_missing_dir += 1
                    continue
                try:
                    n_disk_frames = len(os.listdir(rgb_dir))
                except OSError:
                    skipped_missing_dir += 1
                    continue
                if n_disk_frames < min_frames:
                    skipped_too_few_frames += 1
                    continue
        ...
        if self.check_video_dir and (skipped_missing_dir or skipped_too_few_frames or skipped_short_actions):
            print(
                f"[VLNActionDataset] task_id={self.task_id} filtered episodes -> "
                f"missing_rgb_dir={skipped_missing_dir}, "
                f"too_few_frames(<{min_frames})={skipped_too_few_frames}, "
                f"actions<4={skipped_short_actions}; "
                f"remaining samples={len(self.data_list)}"
            )
    ...
    def __getitem__(self, i):
        try:
            ...
            if n_video_frames == 0:
                raise IndexError(f"empty rgb directory: {video_path}/rgb")
            ...
            # Preserve the original sampling interval, only clamp to valid frame index range
            # so that a corrupted/incomplete rgb folder degrades to reusing the last frame
            # instead of raising IndexError.
            sample_step_ids = np.arange(start_idx, end_idx, interval, dtype=np.int32)
            sample_step_ids = np.clip(sample_step_ids, 0, n_video_frames - 1)
            sample_frames = [os.path.join(video_path, 'rgb', video_frames[j]) for j in sample_step_ids]
            ...
            if time_ids[0] != 0:
                ...
                history_step_ids = np.clip(history_step_ids, 0, n_video_frames - 1)
                history_frames = [os.path.join(video_path, 'rgb', video_frames[j]) for j in history_step_ids]
            else:
                history_frames = []
            ...
            # Gracefully skip corrupted / incomplete episodes instead of crashing the whole run.
            try:
                _ep_id, _ins_id, _start_idx, _valid_idx = self.data_list[i]
                _video_path = self.nav_data[_ep_id].get('video', '<unknown>')
                try:
                    _n_disk = len(os.listdir(os.path.join(_video_path, 'rgb')))
                except OSError:
                    _n_disk = -1
                _actions_len = len(self.nav_data[_ep_id].get('actions', []))
            except Exception:
                _ep_id, _video_path, _n_disk, _actions_len = -1, '<unknown>', -1, -1
            print(
                f"[VLNActionDataset] WARNING: skipping sample idx={i}, ep_id={_ep_id}, "
                f"video={_video_path}, disk_frames={_n_disk}, actions_len={_actions_len}, "
                f"err={type(e).__name__}: {e}"
            )
            # Fall back to the next sample cyclically; safe because at least one valid episode
            # is expected to remain after __init__ filtering.
